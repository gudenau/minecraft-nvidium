...
public class MixinRenderSectionManager implements INvidiumWorldRendererGetter {
    ...
    @Redirect(
        ...
        at = @At(
            value = "INVOKE",
            target = "Lnet/caffeinemc/mods/sodium/client/render/chunk/RenderSection;setTaskCancellationToken(Lnet/caffeinemc/mods/sodium/client/util/task/CancellationToken;)V"
        )
    )
    private void injectEnqueueFalse(RenderSection instance, CancellationToken type) {
        instance.setTaskCancellationToken(type);
        if (Nvidium.IS_ENABLED && Nvidium.config.async_bfs) {
